...
{	
	u1_printf("come into mqtt initial");
...
//	sprintf(ServerIP,"%s","47.106.235.154");              //构建服务器域名183.230.40.39
//	ServerPort = 1883;                                   //服务器端口号6002
	sprintf(ServerIP, "%s", "192.168.43.124");              // 本地服务器IP
	ServerPort = 3010;                                      // 本地端口
	
	u1_printf("IP AND PORT : %s:%d\r\n",ServerIP,ServerPort); //串口输出调试信息
	u1_printf("ID :%s\r\n",ClientID);               //串口输出调试信息
	u1_printf("USERNAME :%s\r\n",Username);               //串口输出调试信息
	u1_printf("PASSWARD :%s\r\n",Passward);               //串口输出调试信息
}
...
void MQTT_DealPushdata_Qs0(unsigned char *redata)
{
	...
	u1_printf("RECV_NUMBER:%d\r\n",pack_num);		      //串口输出信息,接受了多少个数据
	...
	{                                		
		...
		CMDBuf_Deal(tempbuff, cmd_len);                             			   //加入命令到命令缓冲区
